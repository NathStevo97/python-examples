...
for page in range(1, pages + 1): # start at page 1 and increase by 1 each time
    ...
    items = div.find_all(text=re.compile(search_term)) # matches with any items satisfying the search term in some form
    for item in items: # want to get the name and price of each item
        ...
        next_parent = item.find_parent(class_="item-container") # looks for any parent in the tree with the class name
        try:
            price = next_parent.find(class_="price-current").find("strong").string
            items_found[item] = {"price": int(price.replace(",", "")), "link": link} # create dictionary based on items found with price and item link
        except:
            pass
