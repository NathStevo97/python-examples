...
def create_glue_table_from_dataframe(
    glue_client, s3_client, database_name, table_name, dataframe, bucket_name, key
):
    # Create S3 bucket
...
def test_create_glue_table_and_query(
    s3_client, glue_client, athena_client, sqlalchemy_engine
):
    ...
    create_glue_table_from_dataframe(
        glue_client, s3_client, database_name, table_name, df, bucket_name, key
    )
