...
def test_hive_table_exists(mock_aandi_connect):
    ...
    with mock_aandi_connect.engine.connect() as conn:
        table_exists = hive_table_exists(
            conn, mock_aandi_connect.database_name, table_name
        )
    assert table_exists
    # Create a sample DataFrame
    df = pd.DataFrame({"column1": ["data1", "data2"], "column2": ["data3", "data4"]})
    ...
    response = mock_aandi_connect.glue_client.get_table(
        DatabaseName=mock_aandi_connect.database_name, Name=table_name
    )
    assert response["Table"]["Name"] == table_name
    assert (
        response["Table"]["StorageDescriptor"]["Location"]
        == f"s3://{mock_aandi_connect.bucket_name}/{key}"
    )
    with mock_aandi_connect.engine.connect() as conn:
...
def test_create_new_table_from_query(mock_aandi_connect):
    ...
    response = mock_aandi_connect.glue_client.get_table(
        DatabaseName=mock_aandi_connect.database_name, Name=original_table_name
    )
    ...
    response = mock_aandi_connect.glue_client.get_table(
        DatabaseName=mock_aandi_connect.database_name, Name=new_table_name
    )
    ...
    with mock_aandi_connect.engine.connect() as conn:
        table_exists = hive_table_exists(
            conn, mock_aandi_connect.database_name, new_table_name
        )
    assert table_exists

    # List all tables in the database for debugging
    tables = mock_aandi_connect.glue_client.get_tables(
        DatabaseName=mock_aandi_connect.database_name
    )
