...
upgrades = [
    driver.find_element_by_id("productPrice" + str(i)) for i in range(1, -1, -1)
]  # checks the upgrades for each id productPrice0, 1, 2, etc

actions = ActionChains(
    driver
)  # Action chains work in a similar manner to an event queue
actions.click(cookie)  # click based on mouse location
for i in range(5000):
    actions.perform()  # perform all stored actions
    count = int(
        cookie_count.text.split(" ")[0]
    )  # takes the cookie count element and extracts the first part of the element's text before the " "
    print(count)
    for item in upgrades:
        value = int(item.text)
        if (
            value <= count
        ):  # if the value of the upgrade is less than or equal to the total count, run the following
