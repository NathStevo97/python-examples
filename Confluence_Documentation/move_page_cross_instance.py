...
def main():
   ...
   source_response = requests.get(
      source_url,
      headers=headers,
      auth=(os.getenv('NATHAN_PERSONAL_CONFLUENCE_USERNAME'), os.getenv('NATHAN_PERSONAL_CONFLUENCE_PAT'))
   )

   # Define target payload from source response
   json_payload = json.dumps(json.loads(source_response.text), sort_keys=True, indent=4, separators=(",", ": "))
   print(json_payload)

   #target configuration
   targetSpaceKey = "~"
   target_url = f"https://{os.getenv('TARGET_SUBDOMAIN')}.atlassian.net/wiki/rest/api/content"
   headers = {
        "Accept": "application/json",
        "Content-Type": "application/json",
